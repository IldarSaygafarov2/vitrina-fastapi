...
@router.callback_query(F.data.startswith("update_fullname"))
async def update_fullname(call: CallbackQuery, repo: RequestsRepo, state: FSMContext):
    await call.answer()
    ...
    cur_message = await call.message.edit_caption(
        caption=f"Полное имя агента: {realtor.fullname}\nВведите новые имя и фамилию",
        reply_markup=None,
    )
    await state.set_state(RealtorUpdatingState.fullname)
    await state.update_data(realtor_id=realtor_id, realtor_message=cur_message)


@router.message(RealtorUpdatingState.fullname)
async def get_new_fullname(message: Message, repo: RequestsRepo, state: FSMContext):
    data = await state.get_data()
    realtor_id = data.pop("realtor_id")
    cur_message = data.pop("realtor_message")
